...
class _SnakeGameState extends State<SnakeGame> {
  ...
  void _tick() {
    ...
    setState(() {
      ...
      if (newHead.x < 0 || newHead.x >= _cols || newHead.y < 0 || newHead.y >= _rows
        ...
        _running = false; _gameOver = true; _timer?.cancel(); return;
      }
      _snake.insert(0, newHead);
      if (newHead == _food) {
        _score += 10;
        _showHelp = false;
        if (_score % 50 == 0) { _level++; _startTimer(); }
        _spawnFood();
      } else {
        _snake.removeLast();
      }
  ...
  void _spawnFood() {
    Point<int> p;
    do { p = Point(_rng.nextInt(_cols), _rng.nextInt(_rows)); } while (_snake.contains(p));
    _food = p;
  }
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text('🐍 Snake  Lv.$_level  Score:$_score',
              style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
        ),
        body: LeafBackground(
          child: SafeArea(
            child: _gameOver ? _buildGameOver() : !_running ? _buildIntro() : _buildGame(cell),
          ),
        ),
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(s.gameSnakeTitle, style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Color(0xFF2E7D32))),
            const SizedBox(height: 12),
            Text(s.snakeInstructions,
            ...
            ElevatedButton(
              ...
              child: Text('${s.start} 🐍', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            ),
          ],
        ),
      ),
      ...
      children: [
        if (_showHelp)
          Container(
            ...
            child: Text(LanguageProvider.of(context).snakeHint,
                textAlign: TextAlign.center,
        ...
        Expanded(
          child: Padding(
            ...
            child: AspectRatio(
              ...
              child: CustomPaint(
                painter: _SnakePainter(snake: _snake, food: _food, cols: _cols, rows: _rows,
  ...
  Widget _buildControls() {
    ...
    return Column(
      children: [
        _arrowBtn(Icons.keyboard_arrow_up_rounded, () => _setDir(_Dir.up), btnColor),
        Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            _arrowBtn(Icons.keyboard_arrow_left_rounded, () => _setDir(_Dir.left), btnColor),
            const SizedBox(width: 48),
            _arrowBtn(Icons.keyboard_arrow_right_rounded, () => _setDir(_Dir.right), btnColor),
          ],
        ),
        _arrowBtn(Icons.keyboard_arrow_down_rounded, () => _setDir(_Dir.down), btnColor),
      ],
    );
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text('${s.score}: $_score  |  ${s.level}: $_level',
...
class _SnakePainter extends CustomPainter {
  ...
  _SnakePainter({required this.snake, required this.food, required this.cols, required this.rows, required this.foodEmoji});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(Rect.fromLTWH(0, 0, size.width, size.height),
        Paint()..color = const Color(0xFF1B5E20).withOpacity(0.9));
    // Grid
    final gp = Paint()..color = Colors.white.withOpacity(0.05)..strokeWidth = 0.5;
    for (int c = 0; c <= cols; c++) canvas.drawLine(Offset(c * cw, 0), Offset(c * cw, size.height), gp);
    for (int r = 0; r <= rows; r++) canvas.drawLine(Offset(0, r * ch), Offset(size.width, r * ch), gp);
    // Snake
    for (int i = 0; i < snake.length; i++) {
      final s = snake[i];
      final isHead = i == 0;
      canvas.drawRRect(
        RRect.fromRectAndRadius(Rect.fromLTWH(s.x * cw + 1, s.y * ch + 1, cw - 2, ch - 2), const Radius.circular(4)),
        Paint()..color = isHead ? const Color(0xFF76FF03) : const Color(0xFF43A047),
      );
    }
    // Food
    final tp = TextPainter(
      text: TextSpan(text: foodEmoji, style: TextStyle(fontSize: cw * 0.7)),
