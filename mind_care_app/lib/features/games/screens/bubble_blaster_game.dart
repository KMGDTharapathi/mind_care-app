...
_ShootResult _computeShot(_ShootInput input) {
  ...
  for (int step = 0; step < 6000; step++) {
    ...
    if (px < kBubbleR) { px = 2 * kBubbleR - px; vx = -vx; }
    if (px > w - kBubbleR) { px = 2 * (w - kBubbleR) - px; vx = -vx; }

    if (py <= kBubbleR + 4) {
      landRow = 0;
      final cw = cellW();
      landCol = ((px - (landRow.isOdd ? cw / 2 : 0)) / cw)
          .round().clamp(0, kCols - 1);
      placed = true;
      break;
    }
...
class _BubbleShooterState extends State<BubbleShooterGame>
  ...
  Future<void> _shoot(double w, double h) async {
    ...
    _grid.removeWhere((b) => b.row == result.landRow && b.col == result.landCol);
          ...
          child: SafeArea(
            child: Column(
              children: [
                _buildHeader(),
                Expanded(
                  child: _won || _lost ? _buildResult() : _buildGame(),
                ),
              ],
            ),
          ),
  ...
  Widget _buildHeader() {
    return Padding(
      ...
      child: Row(
        children: [
          _statChip(Icons.emoji_events_rounded, 'Score',
              '$_score', const Color(0xFFFFD600)),
          const SizedBox(width: 8),
          _statChip(Icons.bubble_chart_rounded, 'Left',
              '$_remaining / $_totalBubbles', const Color(0xFF69F0AE)),
          const SizedBox(width: 8),
          _statChip(Icons.sports_esports_rounded, 'Shots',
              '$_shots', const Color(0xFFFF6E40)),
          const Spacer(),
          Column(
            children: [
              const Text('NEXT',
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(
                        color: kColors[_nextColor].withOpacity(0.6),
                        blurRadius: 8)
                  ],
                ),
              ),
              ...
              Positioned(
                ...
                child: Center(
                  child: Container(
                    ...
                    child: const Text('Drag or tap above to aim & shoot',
                        style: TextStyle(
    ...
    return Center(
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
                color: (_won ? Colors.green : Colors.red).withOpacity(0.4),
                blurRadius: 40,
                spreadRadius: 5),
          ],
        ),
        child: Column(
          ...
          children: [
            ...
            if (_won && _level < _levels.length - 1)
              _btn('Next Level →', const Color(0xFF69F0AE), Colors.black,
...
class _GridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (aimAngle != null && !firing) {
      for (int i = 0; i < trajectoryDots.length; i++) {
        ...
        canvas.drawCircle(trajectoryDots[i], r,
    ...
    canvas.drawCircle(
        ...
        kBubbleR + 8,
        Paint()
          ..color = kColors[curColor].withOpacity(0.25)
  ...
  void _drawBubble(Canvas canvas, Offset center, Color color,
      {double radius = kBubbleR, double opacity = 1.0}) {
    canvas.drawCircle(
        center + const Offset(1, 2),
        radius,
        Paint()..color = Colors.black.withOpacity(0.3 * opacity));
    ...
    canvas.drawCircle(
        ...
        Paint()
          ..shader = RadialGradient(
            ...
            radius: 0.9,
            colors: [
              Color.lerp(color, Colors.white, 0.35)!.withOpacity(opacity),
              color.withOpacity(opacity),
              Color.lerp(color, Colors.black, 0.25)!.withOpacity(opacity),
            ],
    ...
    canvas.drawCircle(
        ...
        radius * 0.28,
        Paint()..color = Colors.white.withOpacity(0.55 * opacity));
